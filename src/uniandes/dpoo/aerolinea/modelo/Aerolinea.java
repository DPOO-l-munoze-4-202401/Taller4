...
public class Aerolinea
{
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
    	...
    	for(Vuelo vuelo:vuelos) {
    		if(vuelo.getFecha().equals(fecha)&&vuelo.getAvion().getNombre().equals(nombreAvion)) {
    			throw new Exception ("El avión que ha seleccionado ya está ocupado para otro vuelo en el mismo intervalo de tiempo");
    	...
    	for (Ruta r:rutas.values()) {
    		if(r.getCodigoRuta().equals(codigoRuta)) {
    			ruta=r;
    			break;
    		}
    	}
    	if (avion==null||ruta==null){
    		throw new Exception ("El avión o ruta especificados no existen");
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
    	...
    	if(vuelo==null) {
    		throw new Exception ("No existe un vuelo con la fecha y hora especificadas");
    	 ...
    	 int totalTarifa = 0;
    	    for (int i = 0; i < cantidad; i++) {
    	        ...
    	        if (cliente == null) {
    	            throw new Exception("No se encontró el cliente con el identificador proporcionado");
    	        }
    	        int tarifa = calcularTarifaParaCliente(vuelo, cliente);
    	        totalTarifa += tarifa;
